...
-   **Resend**: After a consulting or cobuilder inquiry save, sends two emails: a team notification to `CONTACT_TO_EMAIL` / `CONTACT_TO` (default `hello@explorenovapath.com`, Google Group; Reply-To is the visitor) and a visitor confirmation to the visitor (Reply-To is the team address). Credentials come from `RESEND_API_KEY` or the Replit Resend connector. Optional `CONTACT_FROM_EMAIL` overrides the sender (default `NovaPath Education <hello@novapath.dev>`, the verified novapath.dev identity). Inquiry rows are still saved if either email fails.

### Inquiry delivery (Replit Secrets / Connectors)
Set these on the Education Replit before republishing, then submit a labeled test inquiry. Expect two Resend Delivered rows: team (`hello@explorenovapath.com`) and confirmation (visitor).

1. Connect **Resend** in Replit (same account that sends novapath.dev mail) **or** set `RESEND_API_KEY`.
2. `CONTACT_TO_EMAIL` (optional) — defaults to `hello@explorenovapath.com`. Team notification To and visitor confirmation Reply-To. Set only to override the Google Group destination.
3. `CONTACT_FROM_EMAIL` (optional) — defaults to `NovaPath Education <hello@novapath.dev>`. Used for both emails. Change this after `explorenovapath.com` is verified in Resend if you want Education mail to send from that domain.
